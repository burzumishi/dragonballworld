...
#include "../../Engine/botlib/be_ai_gen.h"
...
// for the voice chats
#include "../UI/menudef/menudef.h" // sos001205 - for q3_ui also
...
void BotSetupForMovement(bot_state_t *bs) {
	...
	if (bs->cur_ps.pm_flags) initmove.presencetype = PRESENCE_CROUCH;
...
float BotAggression(bot_state_t *bs) {
	...
	if (bs->inventory[INVENTORY_HEALTH] < 80) return 0; {
...
/*
	...
	//if not full health
	if (bs->inventory[INVENTORY_HEALTH] < 90) return qfalse; {
		//if the bot has insufficient armor
...
/*
	...
	if (bs->inventory[INVENTORY_HEALTH] < 60) return qfalse;
	//if the bot is low on health
	if (bs->inventory[INVENTORY_HEALTH] < 80) return qfalse; //{
		//if the bot has insufficient armor
//		if (bs->inventory[INVENTORY_ARMOR] < 40) return qfalse;
//	}
	//if the bot can use the bfg
	...
	while((handle = trap_BotNextConsoleMessage(bs->cs, &m)) != 0) {
		...
		if (!BotMatchMessage(bs, m.message)) {
			...
			if (m.type == CMS_CHAT && !bot_nochat.integer) {
				...
				/*
				else if (bs->ainode != BotValidChatPosition(bs) && !TeamPlayIsOn()) {
