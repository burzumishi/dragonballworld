...
static char quake3worldMessage[] = "Welcome to Dragon Ball World";
...
static void ArenaServers_UpdateMenu( void ) {
	...
	if( g_arenaservers.numqueriedservers > 0 ) {
		...
		if( g_arenaservers.refreshservers && ( g_arenaservers.currentping <= g_arenaservers.numqueriedservers ) ) {
			// show progress
			Com_sprintf( g_arenaservers.status.string, MAX_STATUSLENGTH, "%d of %d DBWorld Servers.", g_arenaservers.currentping, g_arenaservers.numqueriedservers);
...
/*
		if( Q_strncmp( "dbw", servernodeptr->gamename, 4) ) {
			continue;
		}
*/
...
/*
		if	(Q_strncmp( "dbw", g_arenaservers.serverlist->gamename, 4) ) {
			break;
		}
*/
...
//	static char leaguebuffer[MAX_LEAGUELENGTH]; // TTimo: unused
	...
	g_arenaservers.banner.string  		= "DBWorld SERVERS";
