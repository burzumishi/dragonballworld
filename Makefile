...
ifndef COPYDIR
COPYDIR="/usr/local/games/DBWorld"
endif
...
ifndef INSTALLDIR
INSTALLDIR="Release.$(PLATFORM).$(ARCH)"
endif
...
ifeq ($(PLATFORM),sunos)
  ...
  INSTALL=ginstall
  MKDIR=gmkdir
  COPYDIR="/usr/local/share/games/DBWorld"
...
ifneq ($(BUILD_SERVER),0)
  TARGETS += $(B)/DBWorldDedicated$(FULLBINEXT)
endif

ifneq ($(BUILD_CLIENT),0)
  TARGETS += $(B)/DBWorld$(FULLBINEXT)
  ifneq ($(BUILD_CLIENT_SMP),0)
    TARGETS += $(B)/DBWorld-smp$(FULLBINEXT)
  endif
endif
...
$(B)/DBWorld$(FULLBINEXT): $(Q3OBJ) $(Q3POBJ) $(LIBSDLMAIN) $(LIBOGG) $(LIBVORBIS) $(LIBVORBISFILE)
	$(echo_cmd) "LD $@"
	$(Q)$(CC) $(CLIENT_CFLAGS) $(CFLAGS) $(CLIENT_LDFLAGS) $(LDFLAGS) \
		-o $@ $(Q3OBJ) $(Q3POBJ) \
		$(LIBSDLMAIN) $(LIBVORBISFILE) $(LIBVORBIS) $(LIBOGG)$(CLIENT_LIBS) $(LIBS)

$(B)/DBWorld-smp$(FULLBINEXT): $(Q3OBJ) $(Q3POBJ_SMP) $(LIBSDLMAIN) $(LIBVORBISFILE) $(LIBVORBIS) $(LIBOGG)
	$(echo_cmd) "LD $@"
...
$(B)/DBWorldDedicated$(FULLBINEXT): $(Q3DOBJ)
	$(echo_cmd) "LD $@"
...
GOBJ_ = \
  ...
  $(B)/Base/Game/ai_cmd.o \
  $(B)/Base/Game/ai_dmnet.o \
  $(B)/Base/Game/ai_dmq3.o \
  $(B)/Base/Game/ai_main.o \
  $(B)/Base/Game/ai_team.o \
  $(B)/Base/Game/ai_vcmd.o \
...
ifneq ($(BUILD_CLIENT),0)
	mkdir $(INSTALLDIR); \
	$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/DBWorld$(FULLBINEXT) $(INSTALLDIR)/DBWorld$(FULLBINEXT)
endif

ifneq ($(BUILD_SERVER),0)
	@if [ -f $(BR)/DBWorldDedicated$(FULLBINEXT) ]; then \
		$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/DBWorldDedicated$(FULLBINEXT) $(INSTALLDIR)/DBWorldDedicated$(FULLBINEXT); \
	fi; \
	$(INSTALL) -m 0755 Tools/Scripts/DBWorldDedicated.bat $(COPYBINDIR)/DBWorldDedicated.bat; \
	$(INSTALL) -m 0755 Tools/Scripts/DBWorldDedicated.sh $(COPYBINDIR)/DBWorldDedicated.sh;
endif

ifneq ($(BUILD_GAME_SO),0)
	mkdir -p $(INSTALLDIR)/DBWorld; \
...
ifneq ($(BUILD_GAME_QVM),0)
	mkdir -p $(INSTALLDIR)/DBWorld/vm; \
	$(INSTALL)  -m 0755 $(BR)/Base/vm/cgame.qvm \
					$(INSTALLDIR)/DBWorld/vm/
...
copyfiles: release
	@if [ ! -d $(COPYDIR)/DBWorld ]; then echo "You need to set COPYDIR to where your DBWorld-Lite data is!"; fi
	-$(MKDIR) -p -m 0755 $(COPYDIR)/DBWorld

ifneq ($(BUILD_CLIENT),0)
	$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/DBWorld$(FULLBINEXT) $(COPYBINDIR)/DBWorld$(FULLBINEXT)
endif

# Don't copy the SMP until it's working together with SDL.
#ifneq ($(BUILD_CLIENT_SMP),0)
#	$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/DBWorld-smp$(FULLBINEXT) $(COPYBINDIR)/DBWorld-smp$(FULLBINEXT)
#endif

ifneq ($(BUILD_SERVER),0)
	@if [ -f $(BR)/DBWorldDedicated$(FULLBINEXT) ]; then \
		$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/DBWorldDedicated$(FULLBINEXT) $(COPYBINDIR)/DBWorldDedicated$(FULLBINEXT); \
	fi; \
	$(INSTALL) -m 0755 Tools/Scripts/DBWorldDedicated.bat $(COPYBINDIR)/DBWorldDedicated.bat; \
	$(INSTALL) -m 0755 Tools/Scripts/DBWorldDedicated.sh $(COPYBINDIR)/DBWorldDedicated.sh;
endif
...
dist:
	rm -rf dbworld(VERSION)
	svn export . dbworld-$(VERSION)
	tar --owner=root --group=root --force-local -cjf dbworld-$(VERSION).tar.bz2 dbworld-$(VERSION)
	rm -rf dbworld-$(VERSION)
